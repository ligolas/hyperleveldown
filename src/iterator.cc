...
void checkEndCallback (Iterator* iterator) {
  ...
  if (iterator->ending) {
    // NanAsyncQueueWorker(iteraIteratorEndtor->endWorker);
    // iterator->endWorker = NULL;

    //delete the leveldb Iterator;
    iterator->IteratorEnd();

     
    iterator->Release();
    iterator->ending=false;
  }
}


//call the database release this object id
void Iterator::Release () {
 ...
 NAN_METHOD(Iterator::New){
  ...
  //Handle the input object and unwrap it to 
...
NAN_METHOD(Iterator::Next){
  ...
  void (*localCallback)(Iterator*)=checkEndCallback;
  std::string key;
  std::string value;
  bool ok;
  ...
  //call the leveldb next function
  ok = iterator->IteratorNext(key, value);

  //
  if(!ok){
    iterator->status=iterator->IteratorStatus();
  }
  ...
  if (ok) {
    ...
    callback->Call(v8::Context::GetCurrent()->Global(),3, argv);
  } else {
...
NAN_METHOD(Iterator::End) {
  ...
  if(iterator->nexting){
    ...
    //delete the iterator id from the database 
    iterator->Release();
    iterator->ending=false;
  }
