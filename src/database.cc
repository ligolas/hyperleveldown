...
NAN_METHOD(Database::Open) {
  ...
  database->status=database->OpenDatabase(options, database->Location());
  if(database->status.ok())
    callback->Call(v8::Context::GetCurrent()->Global(), 0, NULL);
  else {
    v8::Local<v8::Value> argv[] = {
        v8::Exception::Error(v8::String::New("open failed"))
    };
...
NAN_METHOD(Database::Put) {
  ...
  database->status= database->PutToDatabase(options, key,value);
  if(database->status.ok())
    callback->Call(v8::Context::GetCurrent()->Global(), 0, NULL);
  else {
    v8::Local<v8::Value> argv[] = {
        v8::Exception::Error(v8::String::New("put failed"))
    };
    callback->Call(v8::Context::GetCurrent()->Global(), 1,argv);
  }
...
NAN_METHOD(Database::Get) {
  ...
  database->status=database->GetFromDatabase(options, key, value);
  if(database->status.ok()){
    ...
    callback->Call(v8::Context::GetCurrent()->Global(),2, argv);
        ...
        v8::Local<v8::Value> argv[] = {
        v8::Exception::Error(v8::String::New("get failed"))
    };
    callback->Call(v8::Context::GetCurrent()->Global(), 1,argv);
  }
  NanReturnUndefined();
